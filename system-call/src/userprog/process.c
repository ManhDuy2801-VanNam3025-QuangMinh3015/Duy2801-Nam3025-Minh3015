...
process_execute (const char *file_name) 
{
  ...
  tid_t tid;
    ...
    fn_copy0 = palloc_get_page(0);//palloc_get_page(0)
    if (fn_copy0 == NULL)
        return TID_ERROR;
...
process_wait (tid_t child_tid UNUSED)
{
  ...
  while (child_elem_ptr != list_end (l))
  {
   ...
   of the list element. */
    ...
    if (child_ptr->tid == child_tid)//child_tid
    {
      if (!child_ptr->isrun)
      {
      ...
      else //has already been successfully called for the given TID
      {
        return -1;
      }
    }
  ...
  if (child_elem_ptr == list_end (l)) {//child_tid
    return -1;
  }
